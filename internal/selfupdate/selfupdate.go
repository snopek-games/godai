...
const (
	...
	// The asset is named for the distribution; the executable inside it for the
	// command it provides.
	DefaultAssetPrefix = "godai-cli"
	DefaultBinaryName  = "godai"
...
func New(config Config) (*Updater, error) {
	...
	updater := &Updater{
		...
		assetPrefix:     orDefault(config.AssetPrefix, DefaultAssetPrefix),
		binaryName:      orDefault(config.BinaryName, DefaultBinaryName),
		variant:         variant,
...
func (u *Updater) DetectLatest(ctx context.Context) (*Release, error) {
	...
	for _, release := range releases {
		...
		assetName := fmt.Sprintf("%s-%s-%s.zip", u.assetPrefix, u.variant.Name, release.TagName)
...
func (u *Updater) get(ctx context.Context, requestURL string) (io.ReadCloser, error) {
	...
	request.Header.Set("User-Agent", fmt.Sprintf("%s/%s", u.binaryName, u.currentVersion))
...
func (u *Updater) downloadAsset(ctx context.Context, release *Release) (string, string, error) {
	...
	file, err := os.CreateTemp("", "godai-*.zip")
	if err != nil {
		return "", "", err
	}
	defer file.Close()
...
func (u *Updater) unpackExecutable(assetPath string, release *Release, installDir string, mode fs.FileMode) (string, error) {
	...
	wantName := u.binaryName + u.variant.Ext
...
func copyToTempFile(file *zip.File, installDir string, mode fs.FileMode) (destPath string, err error) {
	...
	dest, err := os.CreateTemp(installDir, ".godai-new-*")
	if err != nil {
		return "", err
	}
	defer func() {
...
func reserveTempPath(dir string) (string, error) {
	file, err := os.CreateTemp(dir, ".godai-swap-*")
	if err != nil {
		return "", err
	}
	name := file.Name()
