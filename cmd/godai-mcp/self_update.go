...
func selfUpdateCommand() *cli.Command {
	return &cli.Command{
		Name:  "self-update",
		Usage: "update godai-mcp to the latest release",
		Flags: []cli.Flag{
			&cli.BoolFlag{
				Name:  "check",
...
func runSelfUpdate(ctx context.Context, cmd *cli.Command) error {
	...
	if check && rollback {
		return fmt.Errorf("--check and --rollback are mutually exclusive")
	}
	...
	updater, err := selfupdate.New(selfupdate.Config{
		CurrentVersion:  server.GodaiVersion,
	...
	if !updater.IsNewer(release) {
		fmt.Printf("godai-mcp %s is up-to-date\n", server.GodaiVersion)
		return nil
	}

	if check {
		fmt.Printf("godai-mcp %s is available (currently running %s)\n", release.Version, server.GodaiVersion)
	...
	fmt.Printf("updating %s to godai-mcp %s ...\n", exePath, release.Version)
	...
	fmt.Printf("updated %s to godai-mcp %s\n", exePath, release.Version)
