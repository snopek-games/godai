#!/bin/sh
# Produce a combined test-coverage report for the Go code (./internal/...).
#
...
#
# Usage:
#   scripts/go-coverage.sh            # print merged per-function + total
#   scripts/go-coverage.sh -o cov.out # also write a merged text profile
...
COVPKG="gitlab.com/snopek-games/godai/internal/..."
WORK="$(mktemp -d)"
...
go tool cover -func="$WORK/merged.txt"
