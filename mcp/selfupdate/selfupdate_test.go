...
func assetName(variant, tag string) string {
	return fmt.Sprintf("%s-%s-%s.zip", testAppName, variant, tag)
}

func binaryContent(tag string) string {
	return "godai-mcp " + tag
}
...
func makeReleaseZip(t *testing.T, variant, tag string, omitBinary bool) []byte {
	...
	dir := fmt.Sprintf("%s-%s-%s", testAppName, variant, tag)
	...
	if !omitBinary {
		...
		files = append(files, struct{ name, content string }{
			name:    fmt.Sprintf("%s/%s-%s%s", dir, testAppName, variant, ext),
...
func installTestExecutable(t *testing.T, content string) string {
	...
	exePath := filepath.Join(t.TempDir(), testAppName+"-"+testVariant+testVariantExt)
...
func TestDetectLatest(t *testing.T) {
	...
	is.Equal(release.AssetName, "godai-mcp-linux-x86_64-v0.4.0.zip")
...
func TestFindChecksum(t *testing.T) {
	...
	checksums := "" +
		...
		sum + "  godai-mcp-linux-x86_64-v0.4.0.zip\n" +
		strings.Repeat("b", 64) + " *godai-mcp-macos-arm64-v0.4.0.zip\n"

	got, ok := findChecksum(checksums, "godai-mcp-linux-x86_64-v0.4.0.zip")
	is.True(ok)
	is.Equal(got, sum)

	got, ok = findChecksum(checksums, "godai-mcp-macos-arm64-v0.4.0.zip")
	is.True(ok)
	...
	_, ok = findChecksum(checksums, "godai-mcp-windows-x86_64-v0.4.0.zip")
	is.True(!ok)
}
