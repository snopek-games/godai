...
func TestPackageManagerHint(t *testing.T) {
	...
	npmPath := filepath.Join("/usr", "lib", "node_modules", "@snopek-games",
		"godai-mcp-linux-x64", "bin", "godai-mcp")
	is.True(strings.Contains(PackageManagerHint(npmPath), "npm install"))

	is.Equal(PackageManagerHint(filepath.Join("/usr", "local", "bin", "godai-mcp")), "")
	is.Equal(PackageManagerHint(filepath.Join("/opt", "node_modules_backup", "godai-mcp")), "")
}
...
func TestPermissionErrorHint(t *testing.T) {
	...
	npmErr := &PermissionError{
		Dir:     "/usr/lib/node_modules/@snopek-games/godai-mcp-linux-x64/bin",
		ExePath: "/usr/lib/node_modules/@snopek-games/godai-mcp-linux-x64/bin/godai-mcp",
		Err:     fs.ErrPermission,
	}
	is.True(strings.Contains(npmErr.Hint(), "npm install"))

	plainErr := &PermissionError{Dir: "/usr/local/bin", ExePath: "/usr/local/bin/godai-mcp", Err: fs.ErrPermission}
	if runtime.GOOS == "windows" {
		is.True(strings.Contains(plainErr.Hint(), "Administrator"))
	} else {
		is.True(strings.Contains(plainErr.Hint(), "sudo /usr/local/bin/godai-mcp self-update"))
	}
}
