...
// If releaseVariants and the CI build matrix drift apart, `godai-mcp self-update`
...
// mcp-build job's parallel matrix, which isn't worth a YAML dependency.
...
func TestReleaseNamesMatchWhatCIPublishes(t *testing.T) {
	...
	appName := regexp.MustCompile(`MCP_APP_NAME:\s*"([^"]*)"`).FindStringSubmatch(string(data))
	is.True(appName != nil)
	is.Equal(appName[1], DefaultAppName)

	const tag = "v0.4.0"
	ci := strings.NewReplacer(
		"${MCP_APP_NAME}", appName[1],
		"${CI_COMMIT_TAG}", tag,
	).Replace(string(data))
	...
	assetName := fmt.Sprintf("%s-%s-%s.zip", updater.appName, updater.variant.Name, tag)
	is.Equal(assetName, "godai-mcp-windows-x86_64-v0.4.0.zip")
