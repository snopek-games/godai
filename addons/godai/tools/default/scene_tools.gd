...
class SceneCreate extends DefaultTool:
	func execute(p_input) -> ToolResult:
		...
		if not ClassDB.class_exists(root_node_type):
			return ToolResult.rejected({error = "Unknown 'root_node_type': %s" % root_node_type})

		var root_node = ClassDB.instantiate(root_node_type)
		if not root_node:
			return ToolResult.rejected({error = "Failed to create '%s'" % root_node_type})
		if not root_node is Node:
			if not root_node is RefCounted:
				root_node.free()
			return ToolResult.rejected({error = "'%s' is not a Node type" % root_node_type})
		...
		if err != OK:
			root_node.free()
			return ToolResult.rejected({error = "Failed to pack scene: %s" % error_string(err)})
		...
		if err != OK:
			root_node.free()
			return ToolResult.rejected({error = "Failed to save scene '%s': %s" % [file_path, error_string(err)]})
...
class SceneInstantiate extends DefaultTool:
	func execute(p_input) -> ToolResult:
		...
		if scene_path == edited_scene_root.scene_file_path:
			return ToolResult.rejected({error = "Cannot instantiate a scene into itself"})

		var packed = load(scene_path)
		if not packed is PackedScene:
			return ToolResult.rejected({error = "'%s' is not a scene" % scene_path})

		var node = packed.instantiate(PackedScene.GEN_EDIT_STATE_INSTANCE)
		if not node:
			return ToolResult.rejected({error = "Failed to instantiate '%s'" % scene_path})
