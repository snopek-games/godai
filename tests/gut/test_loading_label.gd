...
func test_base_text_updates_the_text() -> void:
	...
	assert_string_starts_with(_label.label.text, "Cancelling")


func test_animates_only_while_visible() -> void:
	var before: String = _label.label.text
	await wait_seconds(TICK)
	assert_ne(_label.label.text, before, "the dots advance while visible")

	_label.visible = false
	before = _label.label.text
	await wait_seconds(TICK)
	assert_eq(_label.label.text, before, "hidden labels stop animating")
