# Run: godot --headless --path <project> --script res://verify/verify.gd
# Whether the editor was actually restarted doesn't survive into the end
# state; editor_verify.gd checks that against the still-running editor.

extends SceneTree

var _checks: Array = []


func _init() -> void:
	var method := RenderingServer.get_current_rendering_method()
	_check("compatibility_renderer", method == "gl_compatibility", "rendering_method is '%s'" % method)
	_emit()


func _check(name: String, ok: bool, detail: String = "") -> void:
	_checks.append({"name": name, "ok": ok, "detail": detail})


func _emit() -> void:
	var passed: int = 0
	for c in _checks:
		if c["ok"]:
			passed += 1
	var result := {
		"passed": passed == _checks.size(),
		"checks_passed": passed,
		"checks_total": _checks.size(),
		"checks": _checks,
	}
	# Sentinel prefix so the scorer can find this amid Godot's own output.
	print("GODAI_VERIFY_JSON:" + JSON.stringify(result))
	quit(0 if passed == _checks.size() else 1)
